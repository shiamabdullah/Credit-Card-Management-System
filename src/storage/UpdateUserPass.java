 ...
 * @author root
 */
class UpdateUserPass extends JFrame {
    Container c;
    ...
    JTable SuperTable;
    ResultSet rs,rs2;
    String UDATA,oldpass;
    JButton PassReset;
    JButton Back;
    PreparedStatement ps;
    public UpdateUserPass(String text)
    {
        c=getContentPane();
        c.setBackground(Color.gray);
        c.setLayout(null);
        UDATA= loginclass.udata;
        ...
        PassReset.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) 
            {
             ...
             String query1 = "UPDATE `userInfo` SET `pass`=? WHERE `u_id`=?";
             String query2="SELECT `pass` FROM `userInfo` WHERE `u_id`=?";
